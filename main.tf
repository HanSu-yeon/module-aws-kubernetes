...
resource "aws_iam_role" "sk406-003-ms-cluster" {
...
resource "aws_iam_role_policy_attachment" "sk406-003-ms-cluster-AmazonEKSClusterPolicy" {
  policy_arn = "arn:aws:iam::aws:policy/AmazonEKSClusterPolicy"
  role       = aws_iam_role.sk406-003-ms-cluster.name
}


resource "aws_security_group" "sk406-003-ms-cluster" {
...
resource "aws_eks_cluster" "ms-up-running" {
  ...
  role_arn = aws_iam_role.sk406-003-ms-cluster.arn

  vpc_config {
    security_group_ids = [aws_security_group.sk406-003-ms-cluster.id]
    subnet_ids         = var.cluster_subnet_ids
  }

  depends_on = [
    aws_iam_role_policy_attachment.sk406-003-ms-cluster-AmazonEKSClusterPolicy
  ]
}
...
resource "aws_eks_node_group" "ms-node-group" {
  ...
  node_group_name = "sk406-003-microservices"
